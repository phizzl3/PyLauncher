...
or object based on user number selection using a passed *expanded 
list/tuple.

Import it and pass it an *expanded list of tuples/lists. It will return
list[1] for your selection by default. Pass the optional listreturn=
argument an integer to return a different list item. 
"""
...
def display(*args, listreturn=1):
    ...
    Args:
        *args (list/tuple): *Expanded list of list/tuple pairs/groups
        ...
        listreturn (int, optional): List index of which item to be 
    ...
    # (defaults to index[1]) only if the selection can be found in list.
    while True:
        sel = input('\n Selection: ')
